...
function TV() {
    ...
    useEffect(()=>{
        ...
        async function getCrewDetails(){
            try {
                ...
                const response = await fetch(`https://api.themoviedb.org/3/tv/${paramID}/credits?api_key=dcf64f9381584cecb483ff727dc95ff7`)
        ...
        let linkNos= movieDetails?.first_air_date?.split(" ");
    ...
    async function addFavorite(){ 
        ...
        try {
            ...
            //    remove the color from true to not
               if(isFavoriteTrue){
                ...
                await updateDoc ( docRef, {
                    favoritesTV : arrayRemove(`${paramID}`)
                } )
            ...
            const docRef  = doc(db, "users" , user?.uid )
            await updateDoc ( docRef, {
                favoritesTV : arrayUnion(`${paramID}`)
            } )
        ...
        try {
            if(!user){
               ...
               if(isWatchListTrue){
                ...
                const docRef  = doc(db, "users" , user?.uid )
                await updateDoc ( docRef, {
                    watchListTV : arrayRemove(`${paramID}`)
                } )
            ...
            const docRef  = doc(db, "users" , user?.uid )
            await updateDoc ( docRef, {
                watchListTV : arrayUnion(`${paramID}`)
            } )
    ...
    async function checkFavAndList(id){
        ...
        let isWatchList = info.watchListTV.includes(id)
        let isFavorite = info.favoritesTV.includes(id)
            ...
            movieDetails &&  <>
                ...
                <aside className=' py-2 text-white text-center font-afacad text-4xl md:text-5xl '>
                    ...
                    <span className='font-afacad text-2xl md:text-4xl  text-gray-700'> 
                        { (movieDetails && movieDetails?.first_air_date) &&  ` (${year})`  }
                        </span>
                    </aside>
                ...
                <div className="md:text-2xl  flex flex-row flex-wrap items-center justify-center gap-8 text-white py-4 " >
                    <div onClick={addList} className='flex flex-row  gap-2'>
                            <aside className={` rounded-[100%] h-8 md:w-10 md:h-10 grid place-items-center w-8 border-2 border-gray-800  ${isWatchListTrue && "bg-gray-800" } `}> <i class={`fa-solid fa-list ${isWatchListTrue && "text-red-700"}`}></i>  </aside>
                            <p>Add to list </p> 
                     </div>
                ...
                <div onClick={()=>setYoutubeActive(!youtubeActive)} className=' flex flex-row items-center justify-center gap-4'> 
                    <p className=' text-white md:text-3xl'> Play trailer </p>
                    <i class="fa-solid fa-caret-right text-white text-2xl md:text-3xl"></i> 
                </div>
                    {
                       youtubeActive && 
                    ...
                    <i onClick={()=>setYoutubeActive(false)} class=" absolute z-30 text-white text-4xl md:text-4xl right-[0%] top-[5%] md:top-[2%] fa-solid fa-xmark"></i>
                ...
                <div className='py-4 px-4  text-white'>
                    <h1 className='font-afacad text-4xl text-white  md:text-5xl'>Overview</h1>
