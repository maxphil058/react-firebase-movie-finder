  ...
  useEffect(() => {
    ...
    try {
      ...
      unsuscribe = onSnapshot(docRef, (doc) => {
        if (doc.exists()) {
          let myInfo = (doc.data());
          console.log(myInfo.favoritesTV)
          setMovieIdArr(myInfo.favoritesTV);
    ...
    async function fetchMovieDetails() {
      try {
        async function getMovieDetails(id) {
          let response = await fetch(`https://api.themoviedb.org/3/tv/${id}?api_key=dcf64f9381584cecb483ff727dc95ff7`);
...
return (
  ...
  <div className="watch-list-tv-wrapper">
     {   
      ...
      >
        ...
        <div className="flex flex-col gap-2 w-[55%] md:text-xl">
          <div>
            ...
            <p className="text-gray-700 font-afacad">{doc.release_date || doc.first_air_date}</p>
          </div>
